...
The code snippet outlined below depicts the closed-loop optimization workflow as implemented in the system:

\begin{Verbatim}[formatcom=\color{mycodecolor}]
class KernelOptimizer:
    def __init__(self, kernel_name, device, max_iterations, llm_api_key):
        # Initialize components with proper isolation
        self.test_framework = TestFramework(device=device, tolerance=0.1)
        self.archive = KnowledgeArchive(archive_dir="archive")
        self.llm_optimizer = LLMOptimizer(api_key=llm_api_key)
        self.reporter = Reporter(archive=self.archive, 
                                test_framework=self.test_framework)
        
        # Configure kernel-specific functions and parameters
        self.kernel_func = get_kernel_function(kernel_name)
        self.get_tunable_params = get_tunable_params_function(kernel_name)
        self.get_default_params = get_default_params_function(kernel_name)
        self.test_func = get_test_function(kernel_name)
    
    def optimize(self):
        # Load kernel code from isolated file reader
        ...
        baseline_result = self.test_func(current_params, **input_size)
        ...
        for iteration in range(self.max_iterations):
            ...
            result = self.test_func(suggested_params, **input_size)
            
            # Store attempt in archive
            self.archive.store_kernel(
                kernel_name=self.kernel_name,
                kernel_code=kernel_code,
            ...
            if result["correct"] and result["speedup"] > best_result["speedup"]:
                ...
                current_params = best_params  # Revert to best if degraded
            
            # Early stopping on target speedup
        ...
        report = self.reporter.generate_report(
            ...
            output_file=f"reports/{self.kernel_name}_report.txt"
        )
...
This experimental demonstration underscores several advantages of the closed-loop approach. First, by deferring parameter selection to an LLM guided by structured prompts containing complete kernel source code, hardware characteristics and optimization history, the system explores a richer parameter space than hand-crafted heuristics. The LLM's ability to reason about trade-offs between block sizes, warp counts and pipeline stages enables more informed exploration than random or grid search. Second, the persistent knowledge archive creates an antifragile system that improves over time; future optimization runs can query past successful configurations and learn from historical failures, reducing redundant exploration. The archive's comprehensive metadata enables parameter impact analysis, revealing which optimization dimensions contribute most to performance gains. Third, the modular architecture with isolated components (kernel code reader, testing framework, archive, reporter) naturally extends to other kernels and hardware configurations; only the set of tunable parameters, baseline implementations and kernel-specific functions need modification. The file-based kernel code reading system ensures safe operation by avoiding inspection of compiled kernel objects, enabling reliable code extraction for LLM prompts. The testing framework's automated correctness validation and statistical performance analysis provide robust quality assurance. While the current implementation optimizes parameters of existing Triton kernels rather than synthesizing new kernels from scratch, the same closed-loop architecture could be extended to full kernel generation given sufficient context and training. Future work will incorporate automated cost modeling, reinforcement learning techniques and multi-objective optimization to further reduce dependence on manual parameter ranges and improve exploration efficiency.

\section {Results}

The closed-loop optimization system was evaluated on an NVIDIA Tesla T4 GPU with CUDA 12.6 through a complete optimization run targeting the matrix multiplication kernel. The system successfully demonstrated its ability to explore parameter spaces, validate correctness and measure performance through iterative refinement.

\subsection {Baseline Definitions}

Throughout this section, speedup figures are reported relative to the default Triton kernel implementation (Baseline A). Where comparisons to PyTorch are provided, they are clearly marked as PyTorch-relative figures (Baseline B).

\subsection {Optimization Process}

The optimization process executed 10 iterations following baseline establishment. The baseline configuration using default Triton parameters achieved a speedup of 0.053× relative to the PyTorch reference implementation, corresponding to a runtime of 2.052 milliseconds. This baseline performance indicates that the default Triton configuration, while functionally correct, requires optimization to match or exceed PyTorch's highly optimized implementation.

\subsection {Iteration Outcomes}

Across the 10 optimization iterations, the system generated 11 total attempts (including baseline). The LLM suggested diverse parameter configurations exploring different combinations of block sizes, group sizes, pipeline stages and warp counts. Of these attempts, 8 passed correctness verification (72.7\% success rate), while 3 configurations failed due to numerical errors exceeding the tolerance threshold. The incorrect configurations primarily featured larger block sizes (128×128) combined with aggressive pipeline settings, suggesting potential register pressure or memory access pattern issues that led to numerical inaccuracies.

\subsection {Best Configuration}

The optimization process identified an improved configuration achieving a best speedup of 0.083× with a runtime of 1.757 milliseconds, representing a 56.6\% improvement over the baseline runtime. The optimal parameters were: BLOCK\_SIZE\_M=64, BLOCK\_SIZE\_N=32, BLOCK\_SIZE\_K=32, GROUP\_SIZE\_M=1, num\_stages=2, num\_warps=4. This configuration emphasizes moderate block sizes and minimal grouping, suggesting that for the target problem size, simpler configurations with balanced resource utilization outperform more aggressive parameter settings.

\subsection {Performance Analysis}

Parameter impact analysis revealed that changes to BLOCK\_SIZE\_N had the largest average impact on speedup (-0.031×), indicating high sensitivity to the N dimension block size. Changes to num\_warps showed moderate impact (-0.014×), while GROUP\_SIZE\_M, num\_stages and BLOCK\_SIZE\_K demonstrated smaller but measurable effects. The analysis identified several top-performing configurations, with the best achieving 0.083× speedup, followed by configurations with 0.068× and 0.053× speedup respectively. The overall parameter impact values reflect the complexity of kernel optimization and the importance of systematic exploration guided by domain knowledge.

\subsection {Stability Assessment}

Stability analysis across different input sizes revealed limitations in the optimized configuration. While the kernel passed correctness checks for a 512×512×512 matrix multiplication with a speedup of 0.199×, it failed correctness validation for larger problem sizes (1024×1024×1024, 2048×2048×2048 and 4096×4096×4096). This indicates that the parameter configuration, while effective for smaller inputs, requires further tuning or different strategies for larger problem sizes, demonstrating the complexity of achieving portable kernel optimizations across input dimensions. The stability score of 1.000 reflects consistent performance when the kernel is correct, but highlights the need for input-size-aware optimization strategies.

\subsection {System Validation}

The experimental run successfully validated all core system components. The knowledge archive captured all 11 optimization attempts with complete metadata including parameters, speedup metrics, correctness flags and runtime measurements. The LLM optimizer generated diverse parameter suggestions guided by kernel source code and optimization history, demonstrating the effectiveness of structured prompts. The testing framework consistently validated correctness and measured performance with statistical rigor, conducting 5 correctness tests and 100 performance runs per configuration. The reporting system generated comprehensive analysis including parameter impact assessment and stability evaluation. These results demonstrate that the closed-loop architecture functions as designed, providing automated exploration, validation and documentation of kernel optimization processes, while revealing opportunities for further refinement in handling larger input sizes and improving optimization strategies.

\section {Conclusion}
Automatic GPU kernel generation is a challenging problem that requires both algorithmic insight and hardware-specific knowledge. Peer-reviewed systems such as TVM, FlexTensor and the Halide GPU autoscheduler demonstrate that domain-specific compilers with learned cost models can achieve performance competitive with hand-tuned kernels across multiple hardware backends. However, these systems rely on fixed search spaces and cannot easily invent new implementation patterns. Large language models have the capacity to synthesize novel code but must be grounded through domain knowledge and iterative feedback. A closed-loop architecture that integrates LLMs with compilation, testing and performance measurement can leverage the strengths of both approaches. The modular implementation provided here, featuring isolated kernel code management, automated testing frameworks, persistent knowledge archives and comprehensive reporting systems, demonstrates a robust approach to kernel optimization. The file-based kernel code reading system ensures safe operation while the knowledge archive creates an antifragile system that improves through accumulated experience. This work underscores the importance of combining machine learning, program analysis and human knowledge in the pursuit of efficient kernel generation, while demonstrating that structured feedback loops can effectively guide LLMs toward high-performance solutions.
